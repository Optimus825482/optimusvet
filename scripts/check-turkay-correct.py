...
# Son 10 satış
if len(sales_in_range) > 10:
    print("\nSon 10 Satış:")
    for sale in sales_in_range[-10:]:
        code, date, amount, status = sale
        print(f"{code} - {date.strftime('%d.%m.%Y')} - {float(amount):,.2f} TL - {status}")

# Bu tarih aralığındaki tahsilatlar (payments tablosundan transactions ile join)
cur.execute("""
    SELECT t.code, t.date, p.amount, p.method
    FROM payments p
...
if payments_in_range:
    ...
    print("Tahsilat kaydı bulunamadı!")
...
print("\n=== SONUÇ ===")
if abs(sales_diff) < 100 and abs(payments_diff) < 100 and abs(balance_diff) < 100:
    print("✅ Veriler eski sistemle uyumlu!")
else:
    print("⚠️ Veriler arasında önemli farklar var!")
    print(f"   Bu müşteri için yeni sistemde {len(sales_in_range)} satış, {len(payments_in_range)} tahsilat var.")
    print(f"   Eski sistemde 94 satış, 40 tahsilat vardı.")
