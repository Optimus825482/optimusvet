...
print(f"Güncel Bakiye: {customer_balance:,.2f} TL\n")
...
cur.execute("""
    SELECT "invoiceNumber", date, "totalAmount", status
    FROM "Sale"
    WHERE "customerId" = %s
    AND date >= %s
    AND date <= %s
...
total_sales_in_range = sum(Decimal(str(sale[2])) for sale in sales_in_range)
...
for sale in sales_in_range[:10]:
    invoice_number, date, amount, status = sale
    print(f"{invoice_number} - {date.strftime('%d.%m.%Y')} - {amount:,.2f} TL - {status}")
...
cur.execute("""
    SELECT "transactionNumber", date, amount, "paymentMethod"
    FROM "Transaction"
    WHERE "customerId" = %s
    AND type = 'PAYMENT'
    AND date >= %s
...
total_payments_in_range = sum(Decimal(str(payment[2])) for payment in payments_in_range)
...
for payment in payments_in_range:
    transaction_number, date, amount, payment_method = payment
    print(f"{transaction_number} - {date.strftime('%d.%m.%Y')} - {amount:,.2f} TL - {payment_method}")
...
sales_diff = total_sales_in_range - Decimal('68003')
payments_diff = total_payments_in_range - Decimal('58529')
balance_diff = calculated_balance - Decimal('9474')
...
for status, count, total in status_breakdown:
    print(f"{status}: {count} adet - {total:,.2f} TL")
